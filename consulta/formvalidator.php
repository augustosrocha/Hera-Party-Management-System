...
*/
// Implementações
// por Ivanilton Quinto em 09/07/2011 (ivanilton@bol.com.br)
// http://rotinadigital.net:
//***
// Validação de datas no formato brasileiro DD/MM/YYYY;
// Validação básica de dados tipo float (precisa de melhor implementação);
// Substituição das funções eregi() e ereg() (depreciadas a partir do PHP 5.0) pelo prag_match();
// Tradução das mensagens de erro para pt-BR.
//
// Todos os demais créditos do script são reservados ao autor original em:
// http://www.html-form-guide.com/php-form/php-form-validation.html
...
define("E_VAL_REQUIRED_VALUE","Digite um valor para %s");
define("E_VAL_MAXLEN_EXCEEDED","O comprimento máximo foi excedido para %s.");
define("E_VAL_MINLEN_CHECK_FAILED","Por favor, insira entrada com comprimento maior que %d para %s");
define("E_VAL_ALNUM_CHECK_FAILED","Por favor, forneça uma entrada alfa-numérico para %s");
define("E_VAL_ALNUM_S_CHECK_FAILED","Por favor, forneça uma entrada alfa-numérico para %s");
define("E_VAL_NUM_CHECK_FAILED","Por favor, forneça dados numéricos para %s");
define("E_VAL_FLOAT_CHECK_FAILED","Por favor, fornecer dados numéricos e (.) para frações em %s");
define("E_VAL_ALPHA_CHECK_FAILED","Por favor, forneça entrada alfabética para %s");
define("E_VAL_ALPHA_S_CHECK_FAILED","Por favor, forneça entrada alfabética para %s");
define("E_VAL_EMAIL_CHECK_FAILED","Por favor, forneça um endereço de e-mail valido");
define("E_VAL_LESSTHAN_CHECK_FAILED","Digite um valor menor do que f% para %s");
define("E_VAL_GREATERTHAN_CHECK_FAILED","Digite um valor maior do que f% para %s");
define("E_VAL_REGEXP_CHECK_FAILED","Por favor,forneça uma entrada válido para %s");
define("E_VAL_DONTSEL_CHECK_FAILED","Opção incorreta selecionada para %s");
define("E_VAL_SELMIN_CHECK_FAILED","Por favor,selecione as opções mínimas %d para %s");
define("E_VAL_SELONE_CHECK_FAILED","Por favor, selecione uma opção para %s");
define("E_VAL_EQELMNT_CHECK_FAILED","Valor de %s deve ser igual ao de %s");
define("E_VAL_NEELMNT_CHECK_FAILED","Valor de %s não deve ser mesmo que o de %s");
define("E_VAL_INVALID_DATE","s% Data Inválida.");
...
class FormValidator 
{
	...
	function ValidaData($input_value, &$default_error_message,$variable_name)
	{
	$bret = true;
	if (preg_match('/^\d{1,2}\/\d{1,2}\/\d{4}$/i', $input_value)){
	...
	if ($res == 1){
	   return $bret;
			...
			$bret=false;
			$default_error_message = sprintf(E_VAL_INVALID_DATE,$variable_name);
	}
}
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name,$formvariables)
	{
		...
		switch($command)
		{
			...
			case 'data':
						{
							$bret = $this->ValidaData($input_value, $default_error_message,$variable_name);
							break;
						}						
            ...
            case 'float':
						{
							...
							if(false == $bret)
							{
								$default_error_message = sprintf(E_VAL_FLOAT_CHECK_FAILED,$variable_name);
							}
							break;							
						}						
