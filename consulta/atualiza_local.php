...
<?php
	...
	$validator -> addValidation("preco_local", "minlen=4", "O campo preço deve ter, no mínimo, 6 caracteres!");
	...
	if($validator -> ValidateForm())
	{
		...
		$endereco_local = $_POST['end_local'];
		$cidade_local = $_POST['cidade_local'];
		$telefone_local = $_POST['tel_local'];
		...
		$sql = "UPDATE `locais` SET `nome_local`='".$nome_local."',`endereco_local`='".$endereco_local."',`cidade_local`='".$cidade_local."',`telefone_local`='".$telefone_local."',";
		$sql = $sql. "`dono_local`='".$dono_local."',`preco_local`='".$preco_local."',";
		$sql = $sql. "`lotacao_local`='".$lotacao_local."',`observacoes_local`='".$observacoes_local."' WHERE id_local = ".$_SESSION['id_local'];
		...
		else
		{
			...
			header ("Location: consulta_local.php?a=sucesso");
		}
	}
	else	
	{
		...
		header ("Location: form_local_alterar.php?c=erro");
	}
